...
// 中文：示例七(本次新增)——★按气味等级★：要求【5 级"大地"气味】+ 复合底料(水50 + 魔力药水30 + 板油20) → 万能修复溶剂。
...
//       成品效果：泼洒到物件/建筑上时，将其耐久、锋利度及二者上限尽数修复如初。
...
// 中文：示例八(本次新增)——★按气味等级★：要求【5 级"浆果派"气味】+ 单一底料(情欲液 100) → 变性药水。
...
//       成品效果：饮用并消化满 5 单位后，触发一次彻底的性别转换(容貌与性器官一并改变)。
...
// 中文：示例九(本次新增)——★按气味等级★：要求【5 级"炽热的甜香"气味】+ 单一底料(情欲液 100) → 媚药。
//       "炽热的甜香"是【催情酒】配方的气味；带它的现成材料(指向催情酒)有：肌腱[major,3]、玫瑰[med,2]、
//       沼泽烟叶粉[med,2] 等；如 肌腱(3)+玫瑰(2)=5 即满足。
//       底料：情欲液(精液)100(现成试剂 /datum/reagent/erpjuice/cum，无酒 → 成品非酒基)。
//       成品效果：饮者被迫发情并持续高潮，直到与他人交合方能解除(或药力代谢殆尽)；仅对开启 ERP 的玩家生效。
