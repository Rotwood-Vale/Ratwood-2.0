...
/mob/living/carbon/human/update_tod(todd)
	...
	if(client)
		...
		if(!cmode)
			SSdroning.play_area_sound(areal, client)
		SSdroning.play_loop(areal, client)
	...
	switch(todd)
		...
		if("day")
			...
			if(has_status_effect(/datum/status_effect/debuff/sleepytime) && HAS_TRAIT(src, TRAIT_NOSLEEP))
