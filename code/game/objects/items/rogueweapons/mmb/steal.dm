...
/datum/intent/steal/on_mmb(mob/living/carbon/human/victim, mob/living/carbon/human/thief, params)
	...
	if(stealroll > effective_targetperception)
		...
		if(mobsbehind.Find(thief) || victim.IsUnconscious() || victim.eyesclosed || victim.eye_blind || victim.eye_blurry || !(victim.mobility_flags & MOBILITY_STAND))
			...
			if(length(stealpos) > 0)
				...
				victim.log_message("has had \the [picked] stolen by [key_name(thief)]", LOG_ATTACK, color="white", attacker = thief.ckey)
				thief.log_message("has stolen \the [picked] from [key_name(victim)]", LOG_ATTACK, color="white", target = victim.ckey)
	...
	if(stealroll <= 5)
		victim.log_message("has had an attempted pickpocket by [key_name(thief)]", LOG_ATTACK, color="white", attacker = thief.ckey)
		thief.log_message("has attempted to pickpocket [key_name(victim)]", LOG_ATTACK, color="white", target = victim.ckey)
