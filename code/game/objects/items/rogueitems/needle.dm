...
/obj/item/needle/proc/sew(mob/living/target, mob/living/user)
	...
	while(!QDELETED(target_wound) && !QDELETED(src) && \
		...
		doctor.reward_actions(multiplier = 0.1,patrons = /datum/patron/divine/pestra)
		...
		doctor.reward_actions(patrons = /datum/patron/divine/pestra, success_message = "Pestra rewards my efforts in closing wounds.")
