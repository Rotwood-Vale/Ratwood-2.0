...
/obj/item/necro_relics/life_crystal/examine(mob/user)
	. = ..()
...
/obj/item/necro_relics/life_crystal/Initialize(mapload)
	..()
	set_light(2, 2, 1, l_color = "#0D7BD7")

/obj/item/necro_relics/life_crystal/proc/recharge(obj/item/reagent_containers/lux/L, mob/user)
...
/obj/item/necro_relics/life_crystal/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/reagent_containers/lux))
		return recharge(I, user)
	return ..()

/obj/item/necro_relics/life_crystal/attack_self(mob/living/user)
	..()
	if(!user)
		return FALSE
	...
	var/mob/living/carbon/human/species/wight/no_equipment/target = new /mob/living/carbon/human/species/wight/no_equipment(T)
	target.crystal = WEAKREF(src)
	target.key = C.key
	current_charges--
	SSjob.EquipRank(target, "Psydonic Wight", TRUE)
