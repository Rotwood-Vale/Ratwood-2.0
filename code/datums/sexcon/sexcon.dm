...
/datum/sex_controller/proc/cum_onto(mob/living/carbon/human/splashed_user = null, cum_on_face = TRUE)
	...
	if(splashed_user)
		...
		var/datum/charflaw/malodorous/malodorous_flaw = user.get_flaw(/datum/charflaw/malodorous)
		if(malodorous_flaw && !splashed_user.has_flaw(/datum/charflaw/malodorous))
			malodorous_flaw.apply_contact_stink(splashed_user)
...
/datum/sex_controller/proc/cum_into(oral = FALSE, mob/living/carbon/human/splashed_user = null, datum/sex_action/knot_action = null, knot_swap_roles = FALSE, mob/living/carbon/human/knot_btm = null, orifice = SEX_PART_NULL, skip_knot_try = FALSE, consume_charge = TRUE)
	...
	if(receiver_sexcon && (oral || !receiver_sexcon.knotted_status || is_receiver_actively_knotted_to_user))
		...
		var/datum/charflaw/malodorous/user_malodorous_flaw = user.get_flaw(/datum/charflaw/malodorous)
		var/datum/charflaw/malodorous/target_malodorous_flaw = splashed_user.get_flaw(/datum/charflaw/malodorous)
		if(user_malodorous_flaw && !target_malodorous_flaw)
			user_malodorous_flaw.apply_contact_stink(splashed_user)
		else if(target_malodorous_flaw && !user_malodorous_flaw)
			target_malodorous_flaw.apply_contact_stink(user)
		modular_record_collar_receive_event(splashed_user, user)
		if(!oral)
			var/obj/item/organ/testicles/testes = user.getorganslot(ORGAN_SLOT_TESTICLES)
