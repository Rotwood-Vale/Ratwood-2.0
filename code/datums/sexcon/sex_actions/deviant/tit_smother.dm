...
/datum/sex_action/titsmother/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(breasts && breasts.breast_size < 3)
		user.visible_message(span_warning("[user] presses [target]'s face against [user.p_their()] chest!"), vision_distance = (user.m_intent == MOVE_INTENT_SNEAK ? 1 : DEFAULT_MESSAGE_RANGE))
		user.sexcon.show_progress = 0
		return
	user.visible_message(span_warning("[user] smothers [target]'s head under [user.p_their()] tits!"), vision_distance = (user.m_intent == MOVE_INTENT_SNEAK ? 1 : DEFAULT_MESSAGE_RANGE))
	user.sexcon.show_progress = 0
...
/datum/sex_action/titsmother/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	var/obj/item/organ/breasts/breasts = user.getorganslot(ORGAN_SLOT_BREASTS)
	if(breasts && breasts.breast_size < 3)
		user.visible_message(span_warning("[user] eases [target]'s face away from [user.p_their()] chest."), vision_distance = (user.m_intent == MOVE_INTENT_SNEAK ? 1 : DEFAULT_MESSAGE_RANGE))
		return
	user.visible_message(span_warning("[user] pulls [target]'s head out from under [user.p_their()] tits."), vision_distance = (user.m_intent == MOVE_INTENT_SNEAK ? 1 : DEFAULT_MESSAGE_RANGE))
