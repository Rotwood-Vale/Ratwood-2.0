/mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad	//Make this cause giant vine tangled messes
...
/mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad/Initialize()
	src.adjust_skillrank(/datum/skill/combat/unarmed, 4, TRUE)
	. = ..()

/mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad/Move(newloc)	//vine movespeed buff
	.=..()
	if(isturf(newloc))
		var/turf/T = newloc
		if(contains_vines(T))
...
/mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad/proc/contains_vines(var/turf/T)
	for(var/obj/structure/vine/V in T)
		return TRUE
	return FALSE

/mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad/simple_add_wound(datum/wound/wound, silent = FALSE, crit_message = FALSE)	//no wounding the watcher
	return

/mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad/MoveToTarget(list/possible_targets)//Step 5, handle movement between us and our target
...
/mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad/proc/vine()
...
/mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad/death(gibbed)
	..()
...
/obj/effect/proc_holder/spell/self/create_vines/cast(list/targets, mob/living/user = usr)
	if(istype(user, /mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad))
		var/mob/living/simple_animal/hostile/retaliate/rogue/fae/questdryad/treeguy = user
