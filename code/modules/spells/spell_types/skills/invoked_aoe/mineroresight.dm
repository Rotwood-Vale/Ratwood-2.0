...
/obj/effect/proc_holder/spell/invoked/mineroresight/cast(list/targets, mob/living/user)
	...
	for(var/turf/closed/rock/rockturfs in view(checkrange,get_turf(user)))
		if(istype(rockturfs, /turf/closed/rock/random/med) || istype(rockturfs, /turf/closed/rock/copper) || istype(rockturfs, /turf/closed/rock/tin) || istype(rockturfs, /turf/closed/rock/coal))
			found_ore(get_turf(rockturfs), user.client, "shieldsparkles")
			//to_chat(user, span_warning("I see some medium quality stone"))
		if(istype(rockturfs, /turf/closed/rock/random/high) || istype(rockturfs, /turf/closed/rock/cinnabar) || istype(rockturfs, /turf/closed/rock/iron))
			found_ore(get_turf(rockturfs), user.client, "sparks")
			//to_chat(user, span_warning("I see some high quality stone"))
		if(istype(rockturfs, /turf/closed/rock/gold) || istype(rockturfs, /turf/closed/rock/silver) || istype(rockturfs, /turf/closed/rock/gem))
			found_ore(get_turf(rockturfs), user.client, "quantum_sparks")
			//to_chat(user, span_warning("I see some GREAT quality stone"))
		if(istype(rockturfs, /turf/closed/rock/bedrock))
