...
/* Valean Javelin - Phalangite ranged AP projectile with slow
...
/obj/effect/proc_holder/spell/invoked/projectile/valean_javelin
	name = "Valean Javelin"
	...
	clothes_req = FALSE
	range = 15
	projectile_type = /obj/projectile/energy/valean_javelin
	...
	overlay_state = "valean_javelin" // Icon by Prominence. Reversed Valean_Phalanx in a different direction
	invocations = list("Pilum Valeum!")
...
/obj/effect/proc_holder/spell/invoked/projectile/valean_javelin/cast(list/targets, mob/user = usr)
	...
	if(empowered)
		if(blade_class == BCLASS_CUT)
			projectile_type = /obj/projectile/energy/valean_javelin/empowered/arc
		else
			projectile_type = /obj/projectile/energy/valean_javelin/empowered
	else
		if(blade_class == BCLASS_CUT)
			projectile_type = /obj/projectile/energy/valean_javelin/arc
		else
			projectile_type = /obj/projectile/energy/valean_javelin
...
/obj/projectile/energy/valean_javelin
	name = "Valean Javelin"
...
/obj/projectile/energy/valean_javelin/on_hit(target)
	. = ..()
	if(isliving(target))
		...
		L.apply_status_effect(/datum/status_effect/debuff/valean_javelin_slow)
		to_chat(L, span_danger("An arcyne javelin pierces through - my movements are sluggish!"))
		if(firer)
			log_combat(firer, L, "javelin-struck")

/obj/projectile/energy/valean_javelin/empowered
	name = "Empowered Valean Javelin"
	icon_state = "youreyesonly"

/obj/projectile/energy/valean_javelin/arc
	name = "Arced Valean Javelin"
	arcshot = TRUE

/obj/projectile/energy/valean_javelin/empowered/arc
	name = "Empowered Arced Valean Javelin"
	arcshot = TRUE

/datum/status_effect/debuff/valean_javelin_slow
	id = "valean_javelin_slow"
	duration = 4 SECONDS
