/* Valean Phalanx - Phalangite bread-and-butter 3x1 line thrust with pushback
...
/obj/effect/proc_holder/spell/invoked/valean_phalanx
	name = "Valean Phalanx"
	...
	overlay_state = "valean_phalanx" // Icon by Prominence
	releasedrain = 20
	chargedrain = 0
	chargetime = 5
	...
	invocations = list("Phalanx Valea!")
...
/obj/effect/proc_holder/spell/invoked/valean_phalanx/cast(list/targets, mob/user = usr)
	...
	for(var/turf/T in line_turfs)
		for(var/mob/living/victim in T)
			...
			if(spell_guard_check(victim, FALSE, deflected ? null : H))
				...
				continue
			if(empowered)
				arcyne_strike(H, victim, held_weapon, round(damage / 2), def_zone, BCLASS_STAB, spell_name = "Valean Phalanx", skip_message = TRUE)
				arcyne_strike(H, victim, held_weapon, round(damage / 2), def_zone, BCLASS_STAB, spell_name = "Valean Phalanx")
			else
				arcyne_strike(H, victim, held_weapon, damage, def_zone, BCLASS_STAB, spell_name = "Valean Phalanx")
			hit_count++
			already_hit += victim
	...
	log_combat(H, null, "used Valean Phalanx")
	return TRUE