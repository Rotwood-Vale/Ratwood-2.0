/*
	...
	3. Receipts pull only when "attacker" is null. A keyless attacker has no block to carry the forward line.
	4. Rows with no event id and no receipt flag are never pulled, only witnessed.
	5. Reads go through the accessors below, so a stray string degrades to text instead of erroring.
*/
...
/// Small enough for one chunk to fit inside a tick
...
/proc/build_pov_entries(mob/M, list/log_source, all_mobs = FALSE, list/actor_labels = list())
	...
	// must complete before the offscreen pass below: it marks the ids that pass then skips
	for(var/actor_id in actor_logs)
		...
		add_pov_witnessed_entries(., seen_entries, already_added, M.ckey, actor_id, included_events, subject_receipts)
	...
	for(var/actor_id in actor_logs)
		...
		if(!actor_labels[row_class]) // they may have acted nowhere the subject saw, their takes still need a labelled block
...
/proc/add_pov_witnessed_entries(list/output, list/entries, list/already_added, subject_ckey, actor_id, list/included_events, list/subject_receipts)
	for(var/entry in entries)
		CHECK_TICK
...
/proc/add_pov_offscreen_actions(list/output, list/attack_entries, list/already_added, actor_id, list/included_events)
	for(var/entry in attack_entries)
		...
		var/event_id = log_entry_field(value, "event")
		if(log_entry_field(value, "receipt"))
			if(log_entry_field(value, "attacker")) // that attacker has a block of their own to carry this hit
				continue
		else if(!event_id)
			// swings always carry ids. Anything without one is typing, deaths, or other lines that are not an event to pull
			continue
		if(event_id)
			if(included_events[event_id])
				continue
			included_events[event_id] = TRUE
		already_added[entry] = TRUE
		output += list(pov_timeline_row(entry, value, actor_id))
...
/proc/pov_legend(mob/M, focused, pov_mode)
	...
	var/colour_key = "<b>Colours.</b> [M] gold on black, everyone else blue on grey. In someone else's block, a bright attack row landed on [M] and a faded one landed on somebody else.\
		<br><b>Marks.</b> &#8648; &#8650; a floor above or below. A grey arrow points off screen toward it. ~ edge of earshot. (N) tiles away. After a witness name, those same marks place that witness instead. A red &#8592; is a hit taken. In someone else's block, it happened off screen.\
...
/proc/style_pov_entries(list/entries, list/actor_labels, subject_label, focus_href, start_index = 1, total_entries = 0, subject_class = "asubject", subject_ckey = null, pov_mode = null)
	...
	for(var/list/wrapper as anything in entries)
		...
		// marks who it was done to, so highlighting a name also stripes rows where they were on the receiving end
		var/recipient = ""
		var/row_target = log_entry_field(stored, "target")
		if(row_target && row_target != subject_ckey && "a[row_target]" != block_class)
			recipient = " t[row_target]"
		...
		. += "<div class='[block_class] row[recipient][row_dim ? " dim" : ""][is_attack ? " atk" : ""]' style='[row_style] [row_dim]padding:1px 5px;' title='[pov_safe_label(full_key)]'>[line][focus]</div>"
	if(have_block)
		. += "</div>"
