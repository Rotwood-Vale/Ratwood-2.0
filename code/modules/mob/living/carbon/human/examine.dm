	...
	if(name in unknown_names)
		...
		if(any_cum_effect || drip)
			...
			if(!show_detail)
				...
				if(external)
					var/external_intensity = max(external.cum_spurt_intensity, 1)
					if(external.has_dried_up)
						. += span_aiprivradio("[capitalize(m2)] body is covered in dried cum!")
				...
				if(facial)
					...
					if(facial.has_dried_up)
						. += span_aiprivradio("[capitalize(m2)] face is plastered with dried cum!")
				...
				if(creampie && !drip)
					...
					if(creampie.has_dried_up)
						. += span_aiprivradio("[capitalize(m2)] crotch is stained with dried cum!")
				...
				if(drip)
					...
					switch(drip.orifice)
						if(SEX_PART_CUNT)
							. += span_aiprivradio("[m1] [is_heavy ? "gushing cum from [m2] sex" : "trickling cum from [m2] sex"]!")
						if(SEX_PART_ANUS)
							. += span_aiprivradio("[m1] [is_heavy ? "leaking heavily from [m2] ass" : "leaking cum from [m2] ass"]!")
						if(SEX_PART_SLIT_SHEATH)
							. += span_aiprivradio("[m1] [is_heavy ? "leaking heavily from [m2] slit" : "trickling cum from [m2] slit"]!")
						if(SEX_PART_CUNT|SEX_PART_ANUS)
							. += span_aiprivradio("[m1] [is_heavy ? "leaking heavily from both [m2] holes" : "dripping cum from both [m2] holes"]!")
						else
							. += span_aiprivradio("[m1] [is_heavy ? "leaking a heavy load" : "dripping cum from [m2] nethers"]!")
