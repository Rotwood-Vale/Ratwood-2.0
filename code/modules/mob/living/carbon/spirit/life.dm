	...
	if(bodytemperature > BODYTEMP_HEAT_DAMAGE_LIMIT && !HAS_TRAIT(src, TRAIT_RESISTHEAT))
		switch(bodytemperature)
			if(360 to 400)
				...
				apply_damage(HEAT_DAMAGE_LEVEL_1, BURN, burn_flag = BURN_FLAG_FIRE)
			if(400 to 460)
				throw_alert("temp", /atom/movable/screen/alert/hot, 2)
				apply_damage(HEAT_DAMAGE_LEVEL_2, BURN, burn_flag = BURN_FLAG_FIRE)
			if(460 to INFINITY)
				throw_alert("temp", /atom/movable/screen/alert/hot, 3)
				if(on_fire)
					apply_damage(HEAT_DAMAGE_LEVEL_3, BURN, burn_flag = BURN_FLAG_FIRE)
				else
					apply_damage(HEAT_DAMAGE_LEVEL_2, BURN, burn_flag = BURN_FLAG_FIRE)
