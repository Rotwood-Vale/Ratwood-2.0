...
/mob/living/carbon/update_stat()
	...
	if(stat != DEAD)
		...
		if(health <= HEALTH_THRESHOLD_DEAD && !HAS_TRAIT(src, TRAIT_NODEATH))
			...
			return
		if(((blood_volume in -INFINITY to BLOOD_VOLUME_SURVIVE) && !HAS_TRAIT(src, TRAIT_BLOODLOSS_IMMUNE)) || IsUnconscious() || IsSleeping() || getOxyLoss() > 75 || (HAS_TRAIT(src, TRAIT_DEATHCOMA)) || (health <= HEALTH_THRESHOLD_FULLCRIT && !HAS_TRAIT(src, TRAIT_NOHARDCRIT) && !HAS_TRAIT(src, TRAIT_TRUE_CRITICAL_RESISTANCE)))
			stat = UNCONSCIOUS
			if(ishuman(src))
				var/mob/living/carbon/human/H = src
			...
			if(health <= crit_threshold && !HAS_TRAIT(src, TRAIT_NOSOFTCRIT) && !HAS_TRAIT(src, TRAIT_TRUE_CRITICAL_RESISTANCE))
				stat = SOFT_CRIT
			else
				stat = CONSCIOUS
			cure_blind(UNCONSCIOUS_BLIND)
