				...
				if("age")
					...
					if(new_age)
						...
						switch (age)
							...
							if (AGE_OLD)
								to_chat(user, "In a place as lethal as PSYDONIA, the elderly are all but marvels... or beneficiaries of the habitually privileged. (-1 STR, -2 SPE, -1 PER, -2 CON, +1 WIL, +2 INT, +1 FOR)")
						// LETHALSTONE EDIT END
						ResetJobs()
						family = FAMILY_NONE
						to_chat(user, "<font color='red'>Classes reset.</font>")
