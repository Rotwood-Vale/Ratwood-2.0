    ...
    return Object.entries(crafties)
      .filter(
        ([key2, item2]) =>
          !onlyCraftable ||
          craftability.some(
            (object) => object[0] === item2.name && object[1] === 1,
          ),
      )
      .sort(([, aVal], [, bVal]) =>
        String(aVal.name).localeCompare(String(bVal.name)),
      )
      .filter(([id, item]) => {
    ...
    <Stack>
      ...
      <Stack.Item basis="80%">
        <Collapsible
          title={recipe.name}
          style={{
            backgroundColor: craftability.some(
              (object) => object[0] === recipe.name && object[1] === 1,
            )
              ? ''
              : 'grey',
          }}
        >
          <LabeledList>
            ...
            <LabeledList.Item
              label="Craft it!"
              style={{ 'margin-left': '20px', gap: '4px' }}
            >
              <Button
                content="1x"
                onClick={() => {
  ...
  const renderColumn = () => {
    return (
      <Stack vertical>
          ...
          <Stack>
            <Stack.Item>
              <Input
                ...
                autoFocus
                value={searchText}
                onInput={(e) => SearchTextModify(e.target.value.toLowerCase())}
              />
            </Stack.Item>
            <Stack.Item>
        ...
        <Stack.Item>
          {Object.entries(crafting_recipes)
  ...
  return (
    <Window title="Crafting" width={340} height={600} resizeable>
      <Window.Content scrollable>
        <Stack horizontal>{renderColumn()}</Stack>
      </Window.Content>
    </Window>
  );
};
