...
# =================
# What this will do:
# 1) Copy the default config/ files from the read-only volume to /roguetown/config/
# 2) Override files under /roguetown/config/ with files present in /gamecfg/
# 3) Process environment variables passed to the container into /roguetown/config/
...
sed -i -r 's/(#|^)ADDRESS .*/ADDRESS '"$DB_HOST"'/' /roguetown/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_LOGIN .*/FEEDBACK_LOGIN '"$DB_USER"'/' /roguetown/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_PASSWORD .*/FEEDBACK_PASSWORD '"$DB_PASS"'/' /roguetown/config/dbconfig.txt
...
envvar_override "TG_GAME_" "/roguetown/config/game_options.txt"
