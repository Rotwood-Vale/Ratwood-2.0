...
/datum/sex_action/chastityplay/kick_cage/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(HAS_TRAIT(target, TRAIT_CHASTITY_SPIKED))
		...
		user.sexcon_action_message(span_warning("The inward spikes punish every strike, digging deeper with each impact!"))
		...
		if(force >= SEX_FORCE_EXTREME && prob(20))
			...
			if(chest && !chest.has_wound(/datum/wound/cbt))
				...
				if(has_cock && has_cunt)
					...
					user.sexcon_action_message(span_userdanger("[user]'s heel drives [target]'s spiked [device] inward with catastrophic force \u2014 something tears deep through [target.p_their()] groin, blood soaking [target.p_their()] thighs as [target.p_they()] crumple."))
					...
					target.add_splatter_floor(get_turf(target))
					user.sexcon_action_message(span_userdanger("[user]'s heel drives the spiked cage inward with full bodyweight behind it \u2014 the crunch that follows is wrong, deep, and [target.p_they()] fold[target.p_s()] immediately, [target.p_their()] stones wrecked by what just happened to them."))
					chest.add_wound(/datum/wound/cbt)
				else if(has_cunt)
					...
					target.add_splatter_floor(get_turf(target))
					user.sexcon_action_message(span_userdanger("The heel-stomp drives [target]'s spiked belt into [target.p_their()] groin with brutal finality \u2014 something gives inside, [target.p_their()] legs buckling as the damage registers and the blood starts."))
					chest.add_wound(/datum/wound/cbt)
