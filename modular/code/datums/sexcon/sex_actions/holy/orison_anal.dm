...
/datum/sex_action/holy/masturbate_other_anus_orison/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user] offers a quiet orison, directing the energies toward [target]'s butt..."), vision_distance = (user.sexcon.subtle_toggled ? 1 : DEFAULT_MESSAGE_RANGE))
...
/datum/sex_action/holy/masturbate_other_anus_orison/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user] finishes the prayer and stops fingering [target]'s butt."), vision_distance = (user.sexcon.subtle_toggled ? 1 : DEFAULT_MESSAGE_RANGE))
