/datum/sex_action/masturbate_anus_prestidigitation
	name = "Prestidigitation finger butt"
...
/datum/sex_action/masturbate_anus_prestidigitation/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user] starts dancing prestidigitation over [user.p_their()] butt..."), vision_distance = (user.sexcon.do_subtle_action ? 1 : DEFAULT_MESSAGE_RANGE))
...
/datum/sex_action/masturbate_anus_prestidigitation/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective(is_stealth = do_subtle)] drives arcyne pressure into [user.p_their()] butt..."), vision_distance = (do_subtle ? 1 : DEFAULT_MESSAGE_RANGE))
	if(!do_subtle)
...
/datum/sex_action/masturbate_anus_prestidigitation/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user] lets the prestidigitation dissipate from [user.p_their()] butt."), vision_distance = (user.sexcon.do_subtle_action ? 1 : DEFAULT_MESSAGE_RANGE))
