...
/datum/sex_action/masturbate_vagina_prestidigitation/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user] starts tracing prestidigitation over [user.p_their()] cunt..."), vision_distance = (user.sexcon.do_subtle_action ? 1 : DEFAULT_MESSAGE_RANGE))
...
/datum/sex_action/masturbate_vagina_prestidigitation/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective(is_stealth = do_subtle)] paints arcyne tingles over [user.p_their()] cunt..."), vision_distance = (do_subtle ? 1 : DEFAULT_MESSAGE_RANGE))
	if(!do_subtle)
...
/datum/sex_action/masturbate_vagina_prestidigitation/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user] stops [user.p_their()] prestidigitation over [user.p_their()] cunt."), vision_distance = (user.sexcon.do_subtle_action ? 1 : DEFAULT_MESSAGE_RANGE))
