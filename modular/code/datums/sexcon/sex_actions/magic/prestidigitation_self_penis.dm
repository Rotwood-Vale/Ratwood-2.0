/datum/sex_action/masturbate_penis_prestidigitation
	name = "Prestidigitation jerk off"
...
/datum/sex_action/masturbate_penis_prestidigitation/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user] starts guiding prestidigitation over [user.p_their()] cock..."), vision_distance = (user.sexcon.do_subtle_action ? 1 : DEFAULT_MESSAGE_RANGE))
...
/datum/sex_action/masturbate_penis_prestidigitation/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective(is_stealth = do_subtle)] coaxes arcyne heat along [user.p_their()] cock..."), vision_distance = (do_subtle ? 1 : DEFAULT_MESSAGE_RANGE))
	if(!do_subtle)
...
/datum/sex_action/masturbate_penis_prestidigitation/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user] lets the prestidigitation fade from [user.p_their()] cock."), vision_distance = (user.sexcon.do_subtle_action ? 1 : DEFAULT_MESSAGE_RANGE))
